...
## 8. Bundled asset provenance (v0.4.0)

| Asset | Origin | Rights | Attribution |
|---|---|---|---|
| `assets/wallpapers/default.jpg` (default vault home background) | AI-generated by the app owner with **Grok (xAI)**, supplied 2026-09-26; not stock or third-party | xAI consumer terms: user owns outputs, commercial use allowed | "Created with Grok" shown in Settings (bottom, under the version) — TR "Varsayılan arka plan: Created with Grok" |
| Launcher icon, feature graphic | Generated by project scripts (`tool/gen_launcher_icon.py`, `tool/gen_feature_graphic.py`) | own | — |
| Roboto (feature graphic) | Flutter SDK font cache | Apache 2.0 | — |

## 9. Pro opt-in: hide the calculator ⓘ button (v0.4.1, #34)

Settings → "Hesap makinesinde bilgi simgesini gizle" (default **off**) is an explicit user opt-in
that only works while a Pro entitlement is active (`ProEntitlement`; in `play` there is no Pro yet →
locked, "Pro yakında"; `full` uses the test stub). If Pro lapses the ⓘ returns automatically. The
vault nature stays disclosed at install/onboarding (step 2), in the store listing and screenshots;
help stays reachable in vault Settings → "Hesap makinesi girişi hakkında". The ⓘ dialog line
"Pro üyeler bu simgeyi Ayarlar'dan gizleyebilir." is shown only in builds where Pro exists.
